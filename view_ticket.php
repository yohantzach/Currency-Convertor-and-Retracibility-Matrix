...
if ($id) {
    $qry = $conn->query("SELECT t.*,concat(c.lastname,', ',c.firstname,' ',c.middlename) as cname, d.name as dname FROM tickets t inner join customers c on c.id= t.customer_id inner join departments d on d.id = t.department_id  where  t.id = ".$_GET['id']);
    if ($qry->num_rows > 0) {
        $row = $qry->fetch_array();
        foreach($row as $k => $v){
            $$k = $v;
					...
					<div class="container-fluid">
						<div class="row">
							...
							<div class="col-md-6">
								...
								<label for="" class="control-label border-bottom border-primary">Investment Plan</label>
